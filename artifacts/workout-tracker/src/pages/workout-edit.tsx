  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await updateWorkout.mutateAsync({
      ...
      data: {
        ...
        exercises: JSON.stringify(exercises.filter(ex => ex.name?.trim())),
      }
    });
          ...
          <CardContent className="space-y-4">
            {exercises.map((ex, i) => (
              <div key={i} className="space-y-3 p-4 rounded border border-border bg-background/50">
                ...
                {isCrossfit && (
                  <div className="pl-6">
                    <Label className="font-mono text-[10px] uppercase text-muted-foreground">Reps / Round</Label>
                    <Input type="number" value={ex.reps_per_round || ""} onChange={e => updateExercise(i, "reps_per_round", parseInt(e.target.value))} className="font-mono h-8 mt-1 w-32" />
                  </div>
                )}
                {isCardio && (
